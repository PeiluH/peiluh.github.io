...
library(zoo)
hvi_tidy_NA_interpolated <- hvi_tidy %>%  mutate(HVI = na.approx(HVI))
dim(hvi_tidy_NA_interpolated)
```
...
```{r}
# TODO
# Outstanding Residential Mortgage Statistics for States 

ormss <- read_csv("nmdb-outstanding-mortgage-statistics-states-quarterly.csv") %>% 
  mutate(PERIOD = as.Date(as.yearqtr(PERIOD, format = "%YQ%q")))
spec(ormss)
head(ormss, 3)

library(skimr)
wa_ormss <- ormss %>% filter(GEOID == "WA")
skim(wa_ormss)
rest_us_ormss <- ormss %>% filter(GEOID != "WA")
skim(rest_us_ormss)

dim(ormss)
ormss_wide_1 <- ormss %>% 
  pivot_wider(
    id_cols = c(GEONAME, MARKET, SERIESID),
    names_from = PERIOD,  
    values_from =VALUE1
  )
dim(ormss_wide_1)
...
ormss_tot_loans <- ormss %>% filter(SERIESID == "TOT_LOANS", MARKET == "All Mortgages")
ormss_tot_loans_cnw <- ormss_tot_loans %>% filter(GEOID %in% c("WA", "CA"))
g <-ggplot(ormss_tot_loans_cnw, aes(x = PERIOD, y = VALUE1, group = GEOID))
g <- g + geom_line(aes(color = GEOID),
              alpha = 0.8, # Opacity: 0 means fully transparent (invisible)
              linetype = 6, # 6 = twodash, you can check more option using `?linetype` 
              linewidth = 0.8) # linewidth
g <- g +
  labs(
    x = "Year",                
    y = "Number of Mortgages (thousand)",  
    title = "Califronia and Washington Mortgage Trend 2013–2025",
    subtitle = "Data source: NMBD",
  )
g

ggplot(data = ormss_tot_loans %>%
         filter(PERIOD =="2025-04-01
"), aes(x = VALUE1)) +
  geom_histogram(
    bins = 10,        # number of bins 
    color = "white",  # border color
  ...
  labs(title = "Histograms of State Mortgage", x = "Mortgage (2025Q2)", y = "Count of States") +
  theme_minimal()

ormss_tot_upb <- ormss %>% 
  filter(SERIESID == "TOT_UPB", MARKET == "All Mortgages")
ggplot(data = ormss_tot_upb %>%
         filter(
           GEOID %in% c("WA", "NY", "CA")
         )) +
  stat_ecdf(
    aes(x = VALUE1, color = GEOID),) + 
  labs(title = "CDF Mortgage Balance for WA, NY and CA", 
       subtitle = "F(x) = P(X ≤ x)",
       y = "Cumulative Distribution Function (CDF)", 
       x = "Mortgage Balance in billion") +
  theme_minimal() +
      theme(legend.position = "bottom")
```

The three charts reveal significant differences in mortgage market dynamics between California, Washington, and New York. **Figure 1** shows that California maintains substantially higher mortgage volume (~5,500-6,500 thousand mortgages) compared to Washington (~1,200-1,300 thousand), with California experiencing a notable decline from 2013 to 2018 before stabilizing, while Washington remains relatively flat throughout the period. **Figure 2** (histogram of state mortgages in 2025Q2) displays a right-skewed distribution across all US states, with most states clustered below 2,000 thousand mortgages, and only a few high-volume states (like California) forming the long tail above 6,000 thousand—highlighting California's position as an outlier in the national mortgage market. **Figure 3** (CDF of mortgage balance) demonstrates dramatic differences in total mortgage debt: California's cumulative mortgage balance reaches approximately $2 trillion, New York reaches around $700-800 billion, while Washington reaches only about $400-500 billion, with the steeper slopes for WA and NY indicating more concentrated mortgage values compared to California's gradual slope suggesting greater diversity in mortgage sizes. These findings indicate that California's housing market is not only larger in volume but also carries significantly higher total debt, reflecting its higher property values and larger population, while Washington's mortgage market is relatively smaller and more stable, suggesting less volatility but also potentially less robust growth in housing finance activity.
